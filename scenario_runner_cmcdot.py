...
from srunner.scenarios.junction_crossing_risk_estimation import *
from srunner.scenarios.pedestrian_crossing_risk_estimation import *
from srunner.scenarios.abnormal_behaviour_scenario import *
...
SCENARIOS = {
    ...
    "JunctionCrossingRisk": JUNCTION_CROSSING_RISK,
    "PedestrianCrossingRisk": PEDESTRIAN_CROSSING_RISK,
    "AbnormalBehaviourRisk": ABNORMAL_BEHAVIOUR_RISK,
...
PARSER.add_argument('-s', '--save', action='store_true', help='Save the traces')
...
class ScenarioRunner(object):
    ...
    def setup_vehicle(self, model, spawn_point, role_name = "test_vehicle", auto_pilot = False):
        """
        ...
        vehicle.set_autopilot(auto_pilot)

        return vehicle



    def setup_pedestrian(self, model, spawn_point, role_name = "pedestrian", auto_pilot = False):
        ...
        if pedestrian is None:
            sys.exit(
                "Error: Unable to spawn pedestrian {} at {}".format(model, spawn_point))
    ...
    def prepare_actors(self, config):
        ...
        if self.ego_vehicle is None:
            self.ego_vehicle = self.setup_vehicle(config.ego_vehicle.model, config.ego_vehicle.transform, role_name = "zoe")
        else:
        ...
        # for actor in config.other_actors:
            new_actor = self.setup_vehicle(actor.model, actor.transform, role_name = "test_vehicle_"+str(idx))
            self.actors.append(new_actor)
        # pdb.set_trace()
        # spawn all other pedestrians
        for idx,actor in enumerate(config.pedestrians):
            new_actor = self.setup_pedestrian(actor.model, actor.transform, role_name = "pedestrian_"+str(idx))
            self.actors.append(new_actor)



# TODO: put sensor configuratin in config file
