...
class KeepVelocity_fixed(AtomicBehavior):
    ...
    def __init__(self, actor, target_velocity, name="KeepVelocity_fixed"):
        """
        Setup parameters including acceleration value (via throttle_value)
        and target velocity
        """
        super(KeepVelocity_fixed, self).__init__(name)
        self.logger.debug("%s.__init__()" % (self.__class__.__name__))
        # self._control = carla.VehicleControl()
        self._actor = actor
        self._target_velocity = target_velocity
    ...
    def update(self):
        ...
        self._actor.set_velocity(carla.Vector3D(-self._target_velocity,0,0))
        self.logger.debug("%s.update()[%s->%s]" % (self.__class__.__name__, self.status, new_status))
        return new_status
    ...
    def terminate(self, new_status):
        ...
        super(KeepVelocity_fixed, self).terminate(new_status)





class KeepVelocity_pedestrian(AtomicBehavior):

    def __init__(self, actor, target_velocity, heading, name="KeepVelocity_pedestrian"):
        """
        Setup parameters including acceleration value (via throttle_value)
        and target velocity
        """
        super(KeepVelocity_pedestrian, self).__init__(name)
        self.logger.debug("%s.__init__()" % (self.__class__.__name__))
        self._control = carla.WalkerControl()
        self._actor = actor
        self._target_velocity = target_velocity
...
class BasicAgentBehavior(AtomicBehavior):
    ...
    def __init__(self, actor, target_location, name="BasicAgentBehavior"):
        ...
        self.logger.debug("%s.__init__()" % (self.__class__.__name__))
        self._agent = BasicAgent(actor,target_speed = 60)
...
class AutoPilotBehavior(AtomicBehavior):
    ...
    def __init__(self, auto_pilot, target_location, name="AutoPilotBehavior"):
        """
        Setup actor and maximum steer value
        """
        super(AutoPilotBehavior, self).__init__(name)
        self.logger.debug("%s.__init__()" % (self.__class__.__name__))
        self._auto_pilot = auto_pilot
        self._ok = False
        self._target = carla.Location()
        self._target.x = target_location.x
        self._target.y = target_location.y
        self._target.z = 1 #target_location.z

    def update(self):
        new_status = py_trees.common.Status.RUNNING
        if not self._ok:
            self._auto_pilot.set_destination(self._target)
            self._ok = True
        else:   
            self._auto_pilot.tick()

            if self._auto_pilot.done():
                new_status = py_trees.common.Status.SUCCESS
    ...
    def terminate(self, new_status):
        self._auto_pilot.brake()
        super(AutoPilotBehavior, self).terminate(new_status)
...
class DebugBehaviour(AtomicBehavior):
    ...
    def update(self):
        ...
        for e in self._loc_list:
            e.z =1
            self._world_debug.draw_point(e, size=0.1, color=carla.Color(r=0, g=255, b=0, a=125), life_time=1)
